...
7. **Reproducible Docker builds** — the Docker image installs runtime deps from
   `requirements.lock.txt` with `pip install --require-hashes`. The lock file is
   generated by `just lock` (pip-compile --generate-hashes) against Python 3.12
   to match `python:3.12-slim`. Refresh it in any PR that changes
   `[project.dependencies]`. CI's `version-check` job enforces freshness.
8. **Subject sanitization** — NATS subject tokens are sanitized via `_slug()`: spaces become
