...
def test_dockerfile_copies_requirements_lock_before_pip_install() -> None:
    """COPY requirements.lock.txt must appear before the RUN pip install line in the builder stage."""
    lines = _dockerfile_lines()
    copy_idx = next(
        (i for i, ln in enumerate(lines) if "COPY requirements.lock.txt" in ln), None
    )
    pip_idx = next(
        (i for i, ln in enumerate(lines) if _PIP_INSTALL_RE.search(ln)), None
    )
    assert copy_idx is not None, "Dockerfile has no 'COPY requirements.lock.txt' line"
    assert pip_idx is not None, "Dockerfile has no 'pip install' line"
    assert copy_idx < pip_idx, (
        "COPY requirements.lock.txt must appear before RUN pip install in the Dockerfile"
    )


def test_dockerfile_pip_install_uses_hashed_lock_file() -> None:
    """The pip install command must use --require-hashes with requirements.lock.txt."""
    text = DOCKERFILE.read_text()
    assert "--require-hashes" in text, (
        "Dockerfile pip install must use --require-hashes for reproducible builds"
    )
    assert "requirements.lock.txt" in text, (
        "Dockerfile must install from requirements.lock.txt (generated by `just lock`)"
    )
